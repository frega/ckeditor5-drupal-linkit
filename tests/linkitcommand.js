...
import LinkitCommand from '../src/linkitcommand';
	...
	beforeEach( () => {
		return ModelTestEditor.create()
			.then( newEditor => {
				editor = newEditor;
				model = editor.model;
				command = new LinkitCommand( editor );
	...
	describe( 'execute()', () => {
		describe( 'non-collapsed selection', () => {
			it( 'should set `linkHref` attribute to selected text', () => {
				...
				command.execute( { href: 'url' } );

				expect( getData( model ) ).to.equal( 'f[<$text linkHref="url" linkitAttrs="{"href":"url"}">ooba</$text>]r' );
				expect( command.value ).to.equal( 'url' );
			} );
			...
			it( 'should set `linkHref` attribute to selected text when text already has attributes', () => {
				...
				expect( getData( model ) ).to.equal(
					'f[<$text linkHref="url" linkitAttrs="{"href":"url"}">o</$text>' +
					'<$text bold="true" linkHref="url" linkitAttrs="{"href":"url"}">oba</$text>]' +
			...
			it( 'should split text and overwrite attribute value when selection is inside text with `linkHref` attribute', () => {
				...
				expect( getData( model ) ).to.equal(
					...
					'[<$text linkHref="url" linkitAttrs="{"href":"url"}">ob</$text>]' +
			...
			() => {
				...
				expect( getData( model ) ).to.equal( 'f<$text linkHref="other url">o</$text>' +
					'[<$text linkHref="url" linkitAttrs="{"href":"url"}">oba</$text>]r' );
				expect( command.value ).to.equal( 'url' );
			} );
			...
			it( 'should overwrite `linkHref` attribute of selected text only, when selection end inside text with `linkHref` ' +
				...
				expect( getData( model ) ).to.equal( 'f[<$text linkHref="url" linkitAttrs="{"href":"url"}">oob</$text>]' +
					'<$text linkHref="other url">a</$text>r' );
				expect( command.value ).to.equal( 'url' );
			} );
			...
			it( 'should set `linkHref` attribute to selected text when text is split by $block element', () => {
				...
				expect( getData( model ) )
					.to.equal( '<p>f[<$text linkHref="url" linkitAttrs="{"href":"url"}">oo</$text></p>' +
						'<p><$text linkHref="url" linkitAttrs="{"href":"url"}">ba</$text>]r</p>' );
				expect( command.value ).to.equal( 'url' );
			} );
			...
			it( 'should set `linkHref` attribute only to allowed elements and omit disallowed', () => {
				...
				expect( getData( model ) )
					.to.equal( '<p>f[<$text linkHref="url" linkitAttrs="{"href":"url"}">oo</$text><img></img>' +
						'<$text linkHref="url" linkitAttrs="{"href":"url"}">ba</$text>]r</p>' );
				expect( command.value ).to.equal( 'url' );
			} );
		} );
